...
class QuestionShow extends React.Component {
	...
	render() {
		...
		if (question.updateDate.length) {
			...
			const lastEditor = editorArr[editorArr.length - 1];
			const EditorName = (
				<div className="question-editor"> <Link to={`/profile/${lastEditor._id}`}>{lastEditor.firstName} {lastEditor.lastName}</Link>
				</div>
			)

			editor = (
				<div className="show-editor">Last edited by {EditorName} on {editDate}
				</div>
			)
		} else {
			editor = null;
